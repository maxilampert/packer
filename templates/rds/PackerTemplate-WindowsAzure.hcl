...
build {
  ...
  provisioner "file" {
    destination = "${var.working_directory}\\reports"
    direction   = "download"
    max_retries = "2"
    source      = "C:\\Windows\\Temp\\Reports\\"
  }

  provisioner "powershell" {
    inline = ["If (Get-Service -Name RdAgent -ErrorAction SilentlyContinue) { Set-Service RdAgent -StartupType Disabled }", "If (Get-Service -Name WindowsAzureTelemetryService -ErrorAction SilentlyContinue) { Set-Service WindowsAzureTelemetryService -StartupType Disabled }", "If (Get-Service -Name WindowsAzureGuestAgent -ErrorAction SilentlyContinue) { Set-Service WindowsAzureGuestAgent -StartupType Disabled }", "Remove-ItemProperty -Path 'HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Setup\\SysPrepExternal\\Generalize' -Name '*'", "If (Test-Path -Path \"C:\\Apps\") { Remove-Item -Path \"C:\\Apps\" -Recurse -Confirm:$False -ErrorAction \"SilentlyContinue\" }"]
  }
